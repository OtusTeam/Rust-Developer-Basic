...
use crate::{app::App, data::TodoTask};

pub fn render<B: Backend>(frame: &mut Frame<B>, app: &mut App) {
    render_list(frame, frame.size(), &app.task_list, &mut app.task_list_state);
}

pub fn render_list<B: Backend>(frame: &mut Frame<B>, area: Rect, task_list: &[TodoTask], list_state: &mut ListState) {
    let items = task_list
        .iter()
        .map(|task| {
    ...
    let block = Block::default()
        .title(Span::styled("TODO", Style::default().fg(colors::NORD6).add_modifier(Modifier::BOLD)))
        .title_alignment(Alignment::Center);
    let list = List::new(items).block(block).highlight_style(Style::default().bg(colors::NORD10));
